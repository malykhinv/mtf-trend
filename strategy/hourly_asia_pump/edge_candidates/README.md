# Кандидаты на успех

Здесь лежат не просто красивые цифры, а рабочие кандидаты на edge по гипотезе аномальных пампов и их продолжения или выдоха.

Что считаем кандидатом:
- правила едины внутри своей идеи и не опираются на разные сценарии для разных часов;
- есть честный full-year результат;
- понятна логика входа, стопа и выхода;
- известны слабые места и ограничения.

Статусы:
- `Ядро` — чистый и устойчивый слой, который можно держать как базу.
- `Рост` — кандидат на усиление доходности без сильного развала по месяцам.
- `Агрессивный` — вариант с более высокой доходностью, но уже с большей хрупкостью.
- `Разведка` — интересный след, который пока не дотягивает до финальной стратегии.

Текущий список:
- [C01 - Confirmed Core TP5](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C01_confirmed_core_tp5.md)
- [C02 - Confirmed Stack With Tighter Stop](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C02_confirmed_stack_tighter_stop.md)
- [C03 - Aggressive Runner 100+ Equity](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C03_aggressive_runner_100plus.md)
- [C04 - America Short Quarter Hours](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C04_america_short_quarter_hours.md)
- [C05 - Long Asia Status 2026-03-30](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C05_long_asia_status_2026_03_30.md)
- [C06 - Session Short Red Flags Status 2026-03-30](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C06_session_short_redflags_status_2026_03_30.md)
- [C07 - America Short Portfolio Overlay](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C07_america_short_portfolio_overlay.md)
- [C08 - Europe Short Broad Overlay](/C:/Users/Ascf/PycharmProjects/mtf-trend-2/strategy/hourly_asia_pump/edge_candidates/C08_europe_short_broad_overlay.md)
