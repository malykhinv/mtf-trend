...
def build_parser() -> argparse.ArgumentParser:
    ...
    rolling_session_optimizer = subparsers.add_parser(
        "run-rolling-session-optimizer",
        help="Run rolling 30/60d session optimizer over existing large-runner discovery artifacts",
    )
    rolling_session_optimizer.add_argument("--days", type=_positive_int_for("--days"), default=DEFAULT_ANOMALY_LAB_DAYS)
    ...
    anomaly_live2.add_argument("--decision-engine-cycle-budget-ms", type=_positive_int_for("--decision-engine-cycle-budget-ms"), default=1_500)
    ...
    anomaly_live2.add_argument("--rolling-context-maintenance-enabled", type=_str_to_bool, default=True)
    anomaly_live2.add_argument("--rolling-context-maintenance-poll-interval-seconds", type=float, default=10.0)
    anomaly_live2.add_argument("--rolling-context-maintenance-symbol-cooldown-seconds", type=float, default=60.0)
    anomaly_live2.add_argument("--rolling-context-maintenance-lookback-minutes", type=_positive_int_for("--rolling-context-maintenance-lookback-minutes"), default=720)
    anomaly_live2.add_argument("--rolling-context-maintenance-max-symbols-per-cycle", type=_positive_int_for("--rolling-context-maintenance-max-symbols-per-cycle"), default=8)
    anomaly_live2.add_argument("--rolling-context-maintenance-request-sleep-seconds", type=float, default=0.02)
    anomaly_live2.add_argument("--rolling-context-maintenance-active-symbol-ttl-ms", type=_positive_int_for("--rolling-context-maintenance-active-symbol-ttl-ms"), default=60_000)
    anomaly_live2.add_argument("--rolling-context-maintenance-closed-candle-lag-ms", type=_non_negative_int_for("--rolling-context-maintenance-closed-candle-lag-ms"), default=5_000)
    ...
    anomaly_live2.add_argument("--top-growth-symbols-per-cycle", type=_positive_int_for("--top-growth-symbols-per-cycle"), default=16)
    anomaly_live2.add_argument("--top-growth-max-cycle-seconds", type=float, default=3.0)
    anomaly_live2.add_argument("--top-growth-fetch-spacing-seconds", type=float, default=0.02)
    anomaly_live2.add_argument("--execution-order-notional-usdt", type=float, default=12.0, help="Fixed live2 entry notional in USDT. Current live contract uses fixed 12 USDT by default, not risk-based sizing.")
...
def resolve_handler(command_name: str) -> Handler:
    ...
    handlers: dict[str, Handler] = {
        ...
        "run-rolling-session-optimizer": commands.run_rolling_session_optimizer,
