...
- `time_alignment.py` - выравнивание временных меток в формате unix ms (int)
...
#### **12.2. Единый контракт времени:**

- Входные временные метки принимаются только как `int` unix timestamp в миллисекундах (ms) от биржи
- Хранение всех временных меток выполняется только как `int` unix ms без конвертаций
- Сравнение, сортировка и фильтрация времени выполняются только по `int` unix ms
...
5. Применение контроля качества:
   ...
   - Выравнивание OI
   - Проверка и сохранение времени строго в unix ms (int)
...
9. **Обрабатывает время** строго как `int` unix ms от биржи (без таймзон и конвертаций)
...
- **Хранение:** сохраняем только `int` unix ms, без `datetime` и без timezone-normalization.
