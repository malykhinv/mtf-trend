...
def format_message(signal: SetupSignal) -> str:
    ...
    if is_defined(signal.price_growth_pct):
        details.append(f"Цена  ↑ {round(signal.price_growth_pct, 0)}%")
    if is_defined(signal.atr_growth_pct):
        details.append(f"ATR   ↑ {round(signal.atr_growth_pct, 0)}%")
    if is_defined(signal.volume_growth_x):
        details.append(f"Объём ↑ {round(signal.volume_growth_x, 0)}x")
