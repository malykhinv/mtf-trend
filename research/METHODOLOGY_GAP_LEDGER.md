...
| Protocol audit | PARTIAL | Protocol audit exists and prediction-stage horizon consistency is now audited through explicit PASS/FAIL rows for whitelist, registry compatibility, target label column, active H_max, purge, prediction artifact identity, and model metadata identity. Broader forensic artifact recomputation is still incomplete. | Add forensic audit that reads all canonical artifacts and can fail the run. |
