...
| Atlas / discovery layer | IMPLEMENTED | Descriptive atlas writes multi-horizon ATR-normalized response surfaces. A separate generic causal regime atlas now freezes discovery-only coarse bins, selects candidates without verification access, uses same-symbol/calendar-month/activity matched controls, re-estimated week-block bootstrap, within-stratum shuffled labels, BY-FDR, and month/symbol stability. Pump-fade axes are strategy declarations rather than Core branches. | Regenerate on the separated lifecycle dataset from a clean commit before any evidence claim; keep regime artifacts descriptive and out of prediction/EV/simulation inputs. |
| Weekly walk-forward prediction | IMPLEMENTED | Weekly CatBoost + Isotonic exists. Fixed-horizon prediction and controls compute purge from active strategy `H_max`. The separate generic binary Core supports horizon-free event targets and admits train labels only when actual `resolution_time < weekly_model_freeze_time`; it uses chronological event-exclusive 60/20/20 fit/validation/calibration, excludes test-week event groups, freezes one model/calibrator per ISO week, stores raw/calibrated probabilities and the frozen weekly climatology baseline, and explicitly skips unsupported weeks without fallback. Pump-fade t0 V1 and the separate ordinal 1/2/3/5/8/13 state family have now run on the regenerated lifecycle dataset. T0 and every ordinal failed the pre-registered high-probability gates; no all-gate-pass model exists. | Keep the negative results and protocol artifacts immutable. Do not proceed to EV/simulation. New predictive work must add genuinely new causal information or a newly registered target/model protocol; lowering gates or mining these OOS periods is forbidden. |
| Incremental information-family testing | IMPLEMENTED | Baseline and augmented weekly WFA arms are run on identical complete-case rows and compared with paired ISO-week block bootstrap, cluster sign-flips, minimum effect gates, and familywise alpha. Nine local OI features and 24 generic BTC/ETH reference-market features were tested on t0/ordinals 1-2; both families failed incremental and high-probability gates and are excluded. | Preserve negative families. Acquire a genuinely new archived stream before the next protocol; current Binance metrics top-trader/long-short/taker ratios are not persisted in the enriched cache and require a separate causal artifact. |
| Calibration artifacts | IMPLEMENTED | Raw/calibrated probabilities, aggregate metrics, class-wise Brier/ECE metrics, and calibration breakdowns by session, week, month, symbol, systemic regime, market-shock group, alpha-decay bucket, and trigger-age bucket are written and independently audited. Class-wise metrics make the post-normalized one-vs-rest isotonic calibration checkable instead of merely present. | Keep monitoring minimum row counts in real 380d runs. |
...
| Pessimistic trade simulation | IMPLEMENTED | Slippage, fees, next open, stop-first collision, causally confirmed swing trailing, registered partial closes, and canonical `strategy_trade_simulation.csv` artifacts exist. Each policy/fraction is a separate research variant; metrics and anti-overlap are scoped by variant and never pooled into one fictitious portfolio. Simulation now groups candles/funding only for symbols that have eligible decisions, and random/matched controls recompute side-specific RR acceptability after their market-time re-anchor. | Keep simulation forensic, memory-bound symbol filtering, random-control RR, and variant-isolation tests as permanent gates. |
| Controls / placebo | IMPLEMENTED | Control artifacts include random/time/symbol-shuffled placebo labels, global/session/event-time/price-path/volume/BTC baselines, anomaly heuristic baselines, CVD/OI/liquidation ablations, idiosyncratic/systemic subsets, and simulation-level always-no-trade/random-entry/matched-market-time controls. Random and matched simulation controls recompute entry/stop/target/RR from the sampled market time and do not reuse the signal row's stale RR flag. Forensic audit now independently verifies the required control names and simulation control metrics from written artifacts. | Keep required-control forensic tests as permanent gate. |
...
28. `science: add bounded registered single/interaction atlas with complete screening ledger and cross-variant correction` - implemented and development-tested; no interaction evidence candidate passed all gates.
29. `science: add causal resolved-event memory and recurrence-chain split isolation` - implemented and paired development-tested; incremental probability hypothesis rejected.
30. `science: acquire and test event-scoped same-symbol positioning with publication-lag and complete-case controls` - implemented and paired development-tested; incremental probability hypothesis rejected.
31. `science: add and test causal event-path CVD with paired complete-case weekly WFA` - implemented and paired development-tested; incremental probability hypothesis rejected.
32. `science: acquire and test same-symbol premium-index/funding crowding with causal publication contracts` - implemented and paired development-tested; statistically positive T0 increment remains below admission minima and is reserved for forward confirmation.
33. `science: make broad cross-fitted phenotype discovery the primary rare-group search path` - implemented and development-tested with disjoint search/calibration/verification, recurrence-chain isolation, direct and OOF-risk-surrogate leaves, complete screening, 19 full null searches, frozen probabilities, BY-FDR, assignments, coverage, and manifests; 14 medium-probability phenotypes verified, zero reached the 0.70 high-probability gate.
34. `science: retain phenotype research memory without weakening gates` - implemented; every frozen phenotype receives a governed follow-up disposition, failure mode, evidence snapshot, mechanism surface, and forward-only next study while its original scientific status remains immutable.
