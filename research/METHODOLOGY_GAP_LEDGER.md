...
| Expected utility | IMPLEMENTED | EV is computed before simulation from OOS probabilities, causal structural stop/target anchors, fees, slippage, and the shared execution/cost model contract. ATR remains descriptive context only and cannot set a physical barrier. Simulation validates policy ids, anchors, reference prices, triggers, and cost fields before consuming a decision. | Keep EV/simulation structural-policy alignment tests as permanent gate. |
