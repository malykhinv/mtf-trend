...
| Compact all-symbol `run-research` executability | IMPLEMENTED | `run-research <strategy> --days N` creates output automatically, uses the full cache period when `--days` is omitted, auto-scales default IS holdout for short windows, and completed the all-symbol 2d command `python main.py run-research broad_anomaly_v1_h30 --days 2`. Atlas now builds through the strict CSV boundary with vectorized grouping; the same run reached forensic audit with `0` FAIL rows and `8` expected WARN rows from the short smoke window lacking non-empty OOS/simulation proof rows. | Keep compact-run smoke plus full tests as regression gates; improve short-window OOS proof depth separately if needed. |
