...
| Expected utility | IMPLEMENTED_PROXY | MVP1 EV is computed before simulation from OOS probabilities, causal structural stop/target anchors, fees, slippage, and the shared execution/cost model contract, but it is explicitly `utility_model_kind=nature_proxy`, `utility_evidence_status=NON_FINAL`, and `utility_evidence_claim_allowed=false` until realized barrier first-hit/timeout outcome modeling exists. ATR remains descriptive context only and cannot set a physical barrier. Simulation validates policy ids, anchors, reference prices, triggers, and cost fields before consuming a decision. | Keep EV/simulation structural-policy alignment tests and utility proxy metadata forensic checks as permanent gates; do not claim final EV proof from nature-proxy utility. |
