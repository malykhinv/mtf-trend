...
live2 category features must stay in the same units as the shared backtest contract: baseline quote liquidity uses a daily proxy from live 5s pace, prior-whipsaw is compared to a live decision-box range rather than a single 5s micro range, and signal stop/risk uses the decision-box low available at decision time
