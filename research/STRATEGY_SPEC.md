...
P491 planner note: backtest acceleration must not use future runner labels, top-growth winners, exits, PnL, or post-entry replay to decide which pre-entry LTF/1s data to fetch. A coarse HTF/1m prefilter is valid only as an impossibility proof: `impossible` means no exact 15s/30s snapshot inside the coarse window can pass the shared core; `possible` and `not_enough_data` both proceed to exact LTF fetch. The targeted data path is staged: pre-entry seed LTF first, short confirm/next-open signal-entry LTF after exact rolling seeds, and long post-entry replay LTF only after the shared core selects a signal.
...
P499 planner note: P494's full seed-stage shared-core prefilter is too expensive for broad 1d/30d discovery because it computes 24h context/prior-spike/hash work before fetching short confirm windows. Signal-entry planning now uses only a context-free mandatory seed-return gate as a data-loading impossibility proof. All return-possible exact seeds get short confirm/next-open LTF fetch and are then evaluated by the normal shared core. This is conservative: it may fetch more short LTF windows, but it must not drop a signal based on incomplete prefiltering.
...
P498 context note: exact seed-aligned 24h context at every 30s/15s rolling offset would require downloading 24h of subminute candles per exact seed and is not an acceptable acceleration path. The long pre-seed baseline/dormancy/prior-spike context is therefore defined as closed HTF-width context built from cheap fully closed 1m/HTF candles, ending no more than 60s before `seed_open_ms`. Exact aggTrade-derived LTF remains mandatory for the rolling seed and confirmation morphology. This is a data-availability contract, not a profitability filter; if cheap context is incomplete or older than the allowed gap, the core returns a visible data dependency.
