...
- Data-source boundary remains `PARTIAL` until a real full 380d all-symbol local Binance Vision cache/export validation is run and recorded; `cache_export_coverage.csv` and `cache_export_manifest.json` are the proof artifacts for that validation, and the export CLI now has explicit proof gates for expected global span, missing UTC days, missing 1m rows, duplicate 1m rows, optional missing open interest, and default exclusion of fixed-date delivery contracts from perpetual exports.
...
- Data-source boundary is still `PARTIAL` until full 380d all-symbol Binance Vision cache/export validation is recorded with `cache_export_coverage.csv` and `cache_export_manifest.json` generated by the strict export proof gate.
