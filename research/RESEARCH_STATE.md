...
- A deeper reframe was then pursued: instead of the coarse fade binary, target continuous runner-ness = forward maximum favorable excursion (forward-MFE) from each decision to structural death. This is the FIRST clean positive of the study: a leakage-safe weekly WFA ranks it with AUC ~0.62 (vs within-week shuffled null ~0.52, p=0.001) on both dev-internal pseudo-OOS and true OOS, and the top prediction decile realises ~27% forward-MFE vs ~7% at the bottom (true OOS). The model keys on market-logical structure (remaining_to_base, pump_size, price-vs-EMA multi-timeframe, VWAP extension, 48h fade memory). Execution was upgraded to a proper structural ATR-confirmed swing-low trailing stop plus take-profit. The best development config (fresh+vertical pumps x p_runner>=weekly-Q70 x tp25 + 4.0-ATR swing trail) looked spectacular in aggregate — winrate 0.65, median +9.7%, EV ~+9%. BUT the registered stability gate rejected it and every other config (0 of ~160 positive-robust in both Q4 halves), and weekly decomposition across all 55 weeks (Jun 2025-Jun 2026) shows the entire edge is ONE explosive week (2025-W41, +14%); every other week is -1.4%..+1.3%, mostly negative; exactly one explosive week in the whole dataset. The pump-continuation edge is therefore a once-per-dataset lottery, not a compoundable stream: it prints only in rare synchronized alt-pump waves and bleeds ~-0.5..-1%/week between them. A regime detector is impossible at N=1. The runner ranking and the execution mechanics are sound and market-logical; the market simply provided one wave in 13 months. Net triangulation across the whole study: free 1m OHLCV(+OI/aggTrades) data contains no frequent, steady, compoundable edge for this pump phenomenon in either direction. Genuinely new information (paid microstructure) or a different, higher-frequency phenomenon is required.
- Reframed to the pure DISCOVERY question (what distinguishes 30%+ runners on high & stable flow from 3% fizzles), which is not killed by the W41 lottery. On quality-flow anomalies at detection, runner-vs-fizzle discrimination is real and SIGN-STABLE across all three time thirds including the true-OOS Feb-Jun 2026 window (excludes W41): runners have MORE prior-48h pumps (hot coin in a campaign; d~0.39 — this reverses the earlier W41-confounded "fresh is better"), higher price-vs-daily-EMA and VWAP extension (d~0.33), bigger/steeper/more-violent initial impulse (pump_size/verticality/volatility/max-1m d 0.19-0.32), and high trade count from many SMALL trades (retail frenzy, avg-trade-notional d -0.16). Effect sizes are modest and diffuse (combined AUC ~0.62). Conclusion: the discrimination (odds shift ~20%->~50% top decile) is a robust OOS-stable market structure; profit is a separate, narrower problem because the payoff MAGNITUDE when correct is regime-gated. The bot's stated purpose — learning what marks a runner — is met.
