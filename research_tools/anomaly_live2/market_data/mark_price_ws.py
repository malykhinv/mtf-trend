"""Binance futures mark-price WebSocket ingestion for anomaly live2."""
...
BINANCE_FUTURES_ALL_MARK_PRICE_WS_URL = "wss://fstream.binance.com/market/ws/!markPrice@arr@1s"
...
class Live2MarkPriceWsSource:
    """Maintain latest mark/index/funding context in SymbolStateStore.

    This source uses Binance's all-market mark-price stream and filters rows to
    the selected live2 universe. It has no REST repair path and never substitutes
    ticker/last-trade prices for mark price.
    """

    source_id = "binance_futures_all_mark_price_ws"
    endpoint_category = BINANCE_FUTURES_MARKET_ENDPOINT_CATEGORY
    websocket_url = BINANCE_FUTURES_ALL_MARK_PRICE_WS_URL
    ...
    def __init__(
        ...
        *,
        ...
        self._thread = threading.Thread(target=self._run_thread, name="live2-binance-mark-price", daemon=True)
    ...
    def is_ready(self) -> bool:
        return bool(self.status().as_dict(stale_ms=self.stale_ms).get("ready"))

    def status(self) -> Live2MarkPriceWsStatus:
        with self._lock:
    ...
    async def _run_ws_loop(self) -> str | None:
        ...
        async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
            async with session.ws_connect(BINANCE_FUTURES_ALL_MARK_PRICE_WS_URL, heartbeat=20, max_msg_size=8 * 1024 * 1024) as ws:
                ...
                while not self._stop_event.is_set():
                    if self._connection_age_exceeded(connection_started_at_ms):
                        ...
                        await ws.close(code=1000, message=b"planned live2 mark-price rotation")
                        self._set_status("rotating", "planned_rotation:connection_max_age_exceeded", count_disconnect=True)
                        return "planned_rotation"
                    msg = await ws.receive(timeout=30)
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        self._handle_payload(msg.data)
                        if self._current_connection_accepted_payload():
                            self._reset_backoff()
                        ...
                        return "error"
        self._set_status("disconnected", "session_exited", count_disconnect=True)
        return "closed"
    ...
    def _handle_payload(self, raw: str) -> None:
        try:
            ...
            self._record_payload_error(f"json_decode_error:{exc}")
            return
        rows = payload if isinstance(payload, list) else payload.get("data") if isinstance(payload, dict) else None
        if not isinstance(rows, list):
            self._record_payload_error("unexpected_mark_price_payload_shape")
            return
        ...
        for row in rows:
            if not isinstance(row, dict):
                ...
                continue
            rows_received += 1
            market_id = str(row.get("s") or "").upper()
            if not market_id:
                rows_filtered += 1
                continue
            ...
            if symbol is None:
                if self._accept_all_symbols:
                    ...
                    rows_filtered += 1
                    continue
            mark_price = optional_float(row.get("p"))
            if mark_price is None or mark_price <= 0:
                rows_filtered += 1
                continue
            self.state_store.update_mark_price(
                symbol=symbol,
                market_id=market_id,
                received_at_ms=received_at_ms,
                event_time_ms=optional_int(row.get("E")),
                mark_price=mark_price,
                index_price=optional_float(row.get("i")),
                estimated_settle_price=optional_float(row.get("P")),
                funding_rate=optional_float(row.get("r")),
                next_funding_time_ms=optional_int(row.get("T")),
        ...
        with self._lock:
            ...
            elif rows_received > 0 and not self._accept_all_symbols:
                self._last_error = "mark_price_payload_had_no_rows_for_selected_symbols"
        ...
        *,
        ...
        with self._lock:
            ...
            if close_code is not None:
                self._last_close_code = int(close_code)
            if close_reason is not None:
                self._last_close_reason = str(close_reason)
            if exception is not None:
                self._last_exception_type = type(exception).__name__
                self._last_exception_text = str(exception)
    ...
    def _mark_connection_started(self, started_at_ms: int) -> None:
        with self._lock:
            self._connection_started_at_ms = int(started_at_ms)
            self._last_rotation_reason = None

    def _connection_age_exceeded(self, started_at_ms: int) -> bool:
        return (utc_now_ms() - int(started_at_ms)) / 1000.0 >= self.connection_max_age_seconds
