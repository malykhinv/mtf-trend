...
def _protocol_rows(*, input_row_count: int, label_row_count: int) -> list[ProtocolAuditRow]:
    ...
    base_rows = [
        ...
        ProtocolAuditRow(
            ...
            status=AuditStatus.PASS,
            message=f"strategy_state_1m.csv accepted through strict schema boundary for {input_row_count} label input rows",
            artifact="strategy_state_1m.csv",
        ),
        ProtocolAuditRow(
            check_name="future_artifact_schema_boundary",
            status=AuditStatus.PASS,
            message=f"strategy_future_paths.csv accepted through strict schema boundary for {input_row_count} label input rows",
        ...
        ProtocolAuditRow(
            ...
            status=AuditStatus.PASS,
            message=f"wrote {label_row_count} strategy_outcome_labels.csv rows",
            artifact="strategy_outcome_labels.csv",
        ),
        ProtocolAuditRow(
