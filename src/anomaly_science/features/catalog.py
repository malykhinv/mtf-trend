...
def build_default_feature_catalog() -> tuple[FeatureCatalogRow, ...]:
    ...
    rows: list[FeatureCatalogRow] = [
        ...
        _row(
            name="initial_pump_height_core_atr_1440",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.ATR_NORMALIZED,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_INSUFFICIENT_HISTORY,
            description="As-of distance from event open to running high normalized by core_atr_1440.",
        ),
        _row(
            name="post_pump_consolidation_minutes",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            ...
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            dtype="int64",
            description="Minutes elapsed since the as-of running high once shelf high/low are known.",
        ),
        _row(
            name="consolidation_width_ratio",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.DIMENSIONLESS_RATIO,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="As-of shelf width divided by initial pump height in price units.",
        ),
        _row(
            name="shelf_low_asof_t",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.RAW_AUDIT_ONLY,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Raw confirmed structural low level, retained for audit joins only.",
        ),
        _row(
            name="shelf_high_asof_t",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.RAW_AUDIT_ONLY,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Raw confirmed structural high level, retained for audit joins only.",
        ),
        _row(
            name="current_low_minus_shelf_low_core_atr_1440",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.ATR_NORMALIZED,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Current candle low minus confirmed shelf low normalized by core_atr_1440.",
        ),
        _row(
            name="current_close_minus_shelf_low_core_atr_1440",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.ATR_NORMALIZED,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Current close minus confirmed shelf low normalized by core_atr_1440.",
        ),
        _row(
            name="current_high_minus_shelf_high_core_atr_1440",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.ATR_NORMALIZED,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Current candle high minus confirmed shelf high normalized by core_atr_1440.",
        ),
        _row(
            name="minutes_spent_below_shelf",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.TIME_RELATIVE,
            ...
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            dtype="int64",
            description="Closed event-window minutes with close below confirmed shelf low as-of snapshot.",
        ),
        _row(
            name="minutes_since_reclaim",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.TIME_RELATIVE,
            ...
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            dtype="int64",
            description="Minutes since latest causal reclaim of confirmed shelf low, if a reclaim exists as-of snapshot.",
        ),
        _row(
            name="volume_on_sweep_percentile",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Current sweep-candle volume rank within the as-of event window; null when no shelf sweep is present.",
        ),
        _row(
            name="trade_count_on_sweep_percentile",
            family=FeatureFamily.STRUCTURE,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_STATE,
            norm=FeatureNormalization.SELF_HISTORY_RELATIVE,
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Current sweep-candle trade-count rank within the as-of event window; null when unavailable or no sweep is present.",
        ),
        _row(
            name="cvd_change_during_sweep",
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="CVD quote delta divided by quote volume on the current shelf-sweep candle, available only with taker-buy quote data.",
        ),
        _row(
            name="oi_change_during_sweep",
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Latest 5m OI percentage change attached to a current shelf sweep, null when no sweep or OI source is absent.",
        ),
        _row(
            name="liq_intensity_during_sweep",
            ...
            audit=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Current-minute long+short liquidation intensity attached to a shelf sweep.",
        ),
        _row(
