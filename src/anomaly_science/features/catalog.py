...
def build_default_feature_catalog() -> tuple[FeatureCatalogRow, ...]:
    ...
    rows: list[FeatureCatalogRow] = [
        ...
        _row(
            ...
            model=True,
            missing=FeatureMissingPolicy.NULL_IF_INSUFFICIENT_HISTORY,
            description="Rolling 15m correlation between symbol and BTC 1m returns computed as-of state_time.",
        ),
        _row(
        ...
        _row(
            ...
            model=True,
            missing=FeatureMissingPolicy.NULL_IF_INSUFFICIENT_HISTORY,
            description="Rolling 60m correlation between symbol and BTC 1m returns computed as-of state_time.",
        ),
        _row(
            name="symbol_return_minus_btc_return_5m",
            family=FeatureFamily.MARKET_CONTEXT,
            source="anomaly_feature_matrix.csv",
            asof=ASOF_MARKET_CONTEXT,
            norm=FeatureNormalization.BTC_RELATIVE,
            model=True,
            missing=FeatureMissingPolicy.NULL_IF_SOURCE_MISSING,
            description="Symbol 5m return minus BTC 5m return using as-of candles.",
        ),
        _row(
