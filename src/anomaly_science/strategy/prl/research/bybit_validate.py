"""Cross-venue validation on BYBIT: do the 3 setups (sweep/breakdown/breakout) hold on an independent exchange?

Reads Bybit linear-perp klines, runs the SAME setups (beta-neutral) + per-BTC-regime split + per-year,
and prints them next to the known Binance numbers. If the edges + regime homes reproduce on Bybit's
independent universe/microstructure, they are real (not Binance artifacts). IS 2020-2025.
Run: python -m anomaly_science.strategy.prl.research.bybit_validate --n 150
"""

from __future__ import annotations

import argparse
import glob
import os

import numpy as np
import pandas as pd

from anomaly_science.strategy.prl.research.breakdown_short_v2 import zigzag

H1 = ".output/market/bybit/um_futures/klines_1h"
IS_END = pd.Timestamp("2026-01-01", tz="UTC")
COST = 6 / 1e4
COLS = ["open", "high", "low", "close", "volume", "quote_volume"]


def liquid(n):
    vols = []
    for f in glob.glob(f"{H1}/*USDT.parquet"):
        try:
            v = pd.read_parquet(f, columns=["timestamp", "quote_volume"])
        except Exception:
            continue
        ts = pd.to_datetime(v["timestamp"], unit="ms", utc=True); mask = ts < IS_END
        if mask.sum() < 5000:
            continue
        vols.append((os.path.basename(f)[:-8], float(v.loc[mask, "quote_volume"].sum())))
    vols.sort(key=lambda x: -x[1])
    return [s for s, _ in vols[:n]]


def load(syms):
    frames = []
    for s in syms:
        f = f"{H1}/{s}.parquet"
        if not os.path.exists(f):
            continue
        df = pd.read_parquet(f, columns=["timestamp", *COLS])
        df["date"] = pd.to_datetime(df["timestamp"], unit="ms", utc=True)
        df = df[df["date"] < IS_END]
        if df.empty:
            continue
        df["symbol"] = s; frames.append(df[["date", "symbol", *COLS]])
    return pd.concat(frames, ignore_index=True).sort_values(["symbol", "date"]).reset_index(drop=True)


def reg(v, thr=0.12):
    return "bull" if v > thr else ("bear" if v < -thr else "side")


def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--n", type=int, default=150); args = ap.parse_args()
    syms = liquid(args.n)
    print(f"bybit liquid symbols: {len(syms)} | {syms[:6]}")
    panel = load(syms)
    px = lambda c: panel.pivot(index="date", columns="symbol", values=c)
    close, high, low = (px(c) for c in ("close", "high", "low"))
    idx, cols = close.index, close.columns
    print(f"panel: {len(cols)} symbols x {len(idx)} hours, {idx.min().date()}..{idx.max().date()}")
    ret = close.pct_change(fill_method=None); btcs = close["BTCUSDT"] if "BTCUSDT" in cols else close.median(axis=1)
    br = btcs.pct_change(fill_method=None)
    beta = (ret.rolling(168, min_periods=48).cov(br).div(br.rolling(168, min_periods=48).var(), axis=0)).shift(1).clip(0, 3).fillna(1.0).to_numpy()
    btc = btcs.to_numpy()
    pc = close.shift(1)
    atr = (high - low).combine((high - pc).abs(), np.maximum).combine((low - pc).abs(), np.maximum).rolling(24, min_periods=12).mean()
    dh, dl, dc = high.resample("1D").max(), low.resample("1D").min(), close.resample("1D").last()
    dpc = dc.shift(1); dtr = (dh - dl).combine((dh - dpc).abs(), np.maximum).combine((dl - dpc).abs(), np.maximum)
    datr = ((dtr.rolling(14, min_periods=7).mean().shift(1)) / dc.shift(1)).reindex(idx, method="ffill").to_numpy()
    hi_lvl = high.resample("1D").max().rolling(20, min_periods=10).max().shift(1).reindex(idx, method="ffill")
    above = close >= hi_lvl
    poke = (high >= hi_lvl) & (~above.shift(1).fillna(False)) & hi_lvl.notna()
    brk = (above & (~above.shift(1).fillna(False)) & hi_lvl.notna()).to_numpy()
    btc_r = (btcs / btcs.shift(60 * 24) - 1).to_numpy(); coin_r = (close / close.shift(60 * 24) - 1).to_numpy()
    C, Hh, L, A, HL = (x.to_numpy() for x in (close, high, low, atr, hi_lvl))
    pk = poke.to_numpy()
    sweep, bd, bo = [], [], []
    for si in range(len(cols)):
        for i in np.where(pk[:, si])[0]:
            if i + 64 >= len(idx) or not (datr[i, si] > 0) or not np.isfinite(coin_r[i, si]):
                continue
            lv = HL[i, si]; j = None
            for t in range(i, i + 5):
                if C[t, si] < lv:
                    j = t; break
            if j is None:
                continue
            ext = np.nanmax(Hh[i:j + 1, si]); entry = None
            for u in range(j + 1, j + 13):
                if Hh[u, si] >= lv:
                    entry = lv; start = u + 1; break
            if entry is None:
                continue
            dp = datr[j, si]; stop = ext + 0.25 * dp * entry; tp = entry * (1 - dp)
            expx, xb = C[min(start + 48, len(idx) - 1), si], min(start + 48, len(idx) - 1)
            for u in range(start, min(start + 48, len(idx))):
                if C[u, si] >= stop:
                    expx, xb = C[u, si], u; break
                if L[u, si] <= tp:
                    expx, xb = tp, u; break
            bn = beta[j, si] * (btc[xb] / btc[j] - 1.0) - (expx / entry - 1.0) - COST
            sweep.append((bn, reg(btc_r[j]), reg(coin_r[j, si]), idx[j].year))
        SH, SL = zigzag(Hh[:, si], L[:, si], A[:, si], 4)
        below = C[:, si] < SL; fresh = below & ~np.concatenate([[False], below[:-1]])
        for e in np.where(fresh)[0]:
            if e + 48 >= len(idx) or not (C[e, si] > 0) or not np.isfinite(SH[e]) or not (A[e, si] > 0) or not np.isfinite(coin_r[e, si]):
                continue
            entry = C[e, si]; istop = SH[e]
            if istop <= entry:
                continue
            risk = (istop - entry) / entry
            if risk <= 0 or risk > 1.0:
                continue
            active = istop; xb, expx = e + 48, C[e + 48, si]
            for t in range(e + 1, e + 49):
                if np.isfinite(SH[t]):
                    active = min(active, SH[t])
                if C[t, si] > active:
                    xb, expx = t, C[t, si]; break
            bn = beta[e, si] * (btc[xb] / btc[e] - 1.0) - (expx / entry - 1.0) - COST
            bd.append((bn, reg(btc_r[e]), reg(coin_r[e, si]), idx[e].year))
        for i in np.where(brk[:, si])[0]:
            dj = i + 4
            if dj + 48 >= len(idx) or not (C[dj, si] > 0) or not (C[dj, si] >= HL[i, si]) or not np.isfinite(coin_r[dj, si]):
                continue
            raw = C[dj + 48, si] / C[dj, si] - 1.0 - COST
            bo.append((raw, reg(btc_r[dj]), reg(coin_r[dj, si]), idx[dj].year))

    binance = {"sweep": "+0.37 (side +0.85)", "breakdown": "+0.13 (coinBear+btcBear big)", "breakout": "+1.10 (BTCbull +1.96)"}
    for name, rows, home in [("SWEEP(bn)", sweep, "side"), ("BREAKDOWN(bn)", bd, "bear"), ("BREAKOUT(raw)", bo, "bull")]:
        d = pd.DataFrame(rows, columns=["pnl", "btc", "coin", "year"]).replace([np.inf, -np.inf], np.nan).dropna()
        if len(d) == 0:
            print(f"\n{name}: no trades"); continue
        print(f"\n=== {name}: {len(d)} trades  edge={d.pnl.mean()*100:+.3f}%   [Binance ~ {binance[name.split('(')[0].lower()]}]")
        by_reg = "  ".join(f"{r}:{g.pnl.mean()*100:+.2f}%(n{len(g)})" for r, g in d.groupby("btc"))
        print(f"  by BTC-regime: {by_reg}")
        if name.startswith("BREAKDOWN"):
            cb = d[(d.coin == "bear") & (d.btc == "bear")]
            print(f"  coin-bear x BTC-bear: {cb.pnl.mean()*100:+.3f}% (n={len(cb)})")
        ys = " ".join(f"{y}:{g.pnl.mean()*100:+.2f}" for y, g in d.groupby("year"))
        print(f"  by year: {ys}")


if __name__ == "__main__":
    main()
