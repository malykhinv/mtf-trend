...
class PumpFadeOiProbabilityConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.state_ordinals != (1, 2):
            raise ValueError("registered OI state ordinals must be exactly (1, 2)")
        if self.oi_features != PUMP_FADE_OI_MODEL_FEATURES:
            raise ValueError("oi_features must equal the strategy OI feature declaration")
        if self.familywise_alpha <= 0.0 or self.familywise_alpha >= 0.5:
            raise ValueError("familywise_alpha must lie in (0, 0.5)")


def load_pump_fade_oi_probability_config(path: Path) -> PumpFadeOiProbabilityConfig:
    raw: dict[str, Any] = json.loads(path.read_text(encoding="utf-8"))
    for name in ("state_ordinals", "oi_features"):
        if name in raw:
            raw[name] = tuple(raw[name])
    unknown = sorted(set(raw) - set(PumpFadeOiProbabilityConfig.__dataclass_fields__))
    if unknown:
        raise ValueError(f"unknown OI probability config fields: {unknown}")
    return PumpFadeOiProbabilityConfig(**raw)


def run_pump_fade_oi_probability_experiment(
    *,
    nature_path: Path,
    state_lattice_path: Path,
    out_dir: Path,
    config: PumpFadeOiProbabilityConfig,
    repository_root: Path,
    allow_dirty_development: bool = False,
) -> Path:
    if out_dir.exists() and any(out_dir.iterdir()):
        raise ValueError(f"OI probability output must be absent or empty: {out_dir}")
    ...
    for variant_name, input_path, base_config, ordinal in variants:
        shared = replace(
            ...
            required_true_columns=("oi_available",),
            required_finite_columns=config.oi_features,
        ...
        baseline_config = replace(
            ...
            strategy_name=f"{shared.strategy_name}_oi_complete_case_baseline",
        )
        augmented_config = replace(
            shared,
            protocol_freeze_id=f"{shared.protocol_freeze_id}:with_oi",
            strategy_name=f"{shared.strategy_name}_with_oi",
            numeric_features=(*shared.numeric_features, *config.oi_features),
        )
        variant_dir = out_dir / variant_name
        baseline_dir = variant_dir / "baseline"
        augmented_dir = variant_dir / "with_oi"
        run_binary_weekly_walk_forward(
        ...
        written.extend((metrics_path, inference_path, gates_path))
    ...
    summary_path = out_dir / "oi_probability_summary.csv"
    _write_csv(summary_path, pd.DataFrame(summary_rows))
    written.append(summary_path)
    protocol_path = out_dir / "oi_probability_protocol.json"
    _write_json(
        protocol_path,
        {
            ...
            "required_true_columns": ["oi_available"],
            "required_finite_columns": list(config.oi_features),
            ...
            "scientific_scope": "paired incremental OI probability evidence; no EV or trading claim",
        },
    )
    written.append(protocol_path)
    manifest = build_manifest(
        run_id="pump-fade-oi-probability-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"),
        artifact_paths=tuple(dict.fromkeys(written)),
        root=out_dir,
    )
    write_manifest(out_dir / "oi_probability.manifest.json", manifest)
    return out_dir
