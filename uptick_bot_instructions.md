...
- `VOL_SPIKE_MULT = 2.0`
- `DEPTH_STREAM_INTERVAL_MS = 250`  # частота потоков стакана
- `DEPTH_SNAPSHOT_LIMIT = 100`  # количество уровней в REST-снапшоте

Рекомендуемые профили массового мониторинга:

- **Умеренный** — поток `depth@500ms`, снапшот на `100` уровней.
- **Интенсивный** — поток `depth@250ms`, снапшот на `100` уровней.
