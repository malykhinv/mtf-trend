...
- Live-режим удерживает главный поток до `Ctrl+C`: после запуска в лог пишется подсказка,
  завершение инициируется через `Orchestrator.stop()`, что закрывает поток данных и дневник
  с финальным сообщением о корректном завершении.
